...
class NodeFile(Entity):
    ...
    def upload(self, payload, auth=None):
        ...
        if ('fileName' not in payload):
            self.logger.error("fileName or fileType not set in payload.")
            return False
        # -- Check if file exists
        if not os.path.isfile(payload['fileName']):
            self.logger.error("File not found: %s", payload['fileName'])
            return False
    ...
    def latest_by_project(self, projectId, auth=None):
        ...
        self.logger.info("LATEST_BY_PROJECT: %s", url)
        self.logger.info("%s %s", r.status_code, r.headers['content-type'])
        self.logger.info(r.text)
        return r
    ...
    def latest_by_project_and_label(self, projectId, label, auth=None):
        ...
        headers = {'Accept': 'application/json'}
        r = requests.get(url, auth=auth, headers=headers)
        self.logger.info("LATEST_BY_PROJECT_AND_LABEL: %s", url)
        self.logger.info("%s %s", r.status_code, r.headers['content-type'])
        self.logger.info(r.text)
        return r
    ...
    def delete_hours_older(self, hours, auth=None):
        ...
        self.logger.info("DELETE_HOURS_OLDER: %s", url)
        self.logger.info("%s %s", r.status_code, r.headers['content-type'])
        self.logger.info(r.text)
        return r
